...
export function ExploreFeedPage() {
  ...
  useEffect(() => {
    ...
    const loadFeed = async () => {
      try {
        ...
        setFeedError(error?.response?.status === 404 ? t('student.explore.feedNotAvailable') : t('student.explore.feedLoadError'));
      }
    };
    ...
    const loadRecommended = async (params = {}) => {
      try {
        ...
        setRecommendedError(error?.response?.status === 404 ? t('student.explore.recommendedNotAvailable') : t('student.explore.recommendedLoadError'));
      }
    };
  ...
  const confirmUseMyLocation = () => {
    if (!navigator.geolocation) {
      setRecommendedError(t('student.geolocationUnsupported'));
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          setRecommended([]);
          setRecommendedLoadError(true);
          setRecommendedError(t('student.explore.recommendedByLocationError'));
        } finally {
      ...
      () => {
        setLocationLoading(false);
        setShowGeoPrompt(false);
        setRecommendedError(t('student.geolocationAccessError'));
      },
      {
    ...
    <PageContainer className="space-y-4">
      ...
      <SectionCard title={t('student.explore.publicFeedTitle')}>
        {feed.length ? (
          ...
          <EmptyState icon={FileText} title={feedLoadError ? t('student.explore.feedErrorTitle') : t('student.explore.feedEmptyTitle')} description={feedError || t('student.explore.feedEmptyDescription')} />
        )}
      </SectionCard>

      <SectionCard title={t('student.explore.recommendedTitle')}>
        <div className="mb-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {recommended.map((trainer) => (
              <div key={trainer.trainerId} className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                {trainer.distanceKm != null && <p className="mt-1 text-xs text-indigo-600">{t('student.distanceKm', { value: trainer.distanceKm })}</p>}
                {!trainer.distanceKm && location.latitude && <p className="mt-1 text-xs text-slate-400">{t('student.explore.distanceUnavailable')}</p>}
              </div>
            ))}
          </div>
        ) : (
          <EmptyState icon={FileText} title={recommendedLoadError ? t('student.explore.recommendedErrorTitle') : t('student.explore.recommendedEmptyTitle')} description={recommendedError || t('student.explore.recommendedEmptyDescription')} />
        )}
      </SectionCard>
    </PageContainer>
  );
